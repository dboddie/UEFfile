...
__version__ = '0.42 (Wed 19th November 2003)'
...
def chunk(f, n, data):
    """chunk(file, number, data)
    
    Write a chunk to the file specified by the open file object, chunk
    number and data supplied.
    """
...
def read_block(chunk):
    ...
    Read a data block from a tape chunk and return the program name, load
    and execution addresses, block data, block number and whether the block
    is supposedly the last in the file.
    """
...
def write_block(f, name, load, exe, length, n):
    """write_block(file, name, load, exe, length, n)
    
    Read data from a file and write it to a string as a file data block in
    preparation to be written as chunk data to a UEF file.
    """
...
def find_next_chunk(chunks, pos, IDs):
    """position, chunk = find_next_chunk(chunks, pos, IDs)
    
    Find the next chunk from the position specified which has an ID in the
    list of IDs given.
    """
...
def find_next_block(chunks, pos):
    """position = find_next_block(chunks, pos)
...
def find_file_start(chunks, pos):
    """position = find_file_start(chunks, pos)
...
def find_file_end(chunks, pos):
    """position = find_file_end(chunks, pos)
...
def read_uef_details(chunks):
    """orig, machine, kbd, emulator, features = read_uef_details(chunks)
    
    Return details about the UEF file and its contents in the form of a
    sequence of values describing the originator, machine, keyboard,
    emulator and any other features.
    """
...
def write_uef_header(file, major, minor):
    """write_uef_header(file, major, minor)
...
def write_uef_creator(file, originator):
    """write_uef_creator(file, originator)
...
def write_machine_info(file, machine, keyboard):
    """write_machine_info(file, machine, keyboard)
    ...
    machines = \
    {
        'BBC Model A': 0, 'Electron': 1, 'BBC Model B': 2, 'BBC Master': 3
    }
...
def write_chunks(file, chunks):
    ...
    Write all the chunks in the list to a file. Saves having loops in other
...
def encode_chunks(file_names):
    ...
    """chunks = encode_chunks(file_names)
    
    Encode data stored in the file names given as chunks for use in UEF files.
    ...
    encode_as = \
    {
        'creator': 0x0, 'originator': 0x0, 'instructions': 0x1, 'manual': 0x1,
...
def export_file(out_path, chunks, name, write_name, load, exe, length):
    ...
    """export_file(out_path, chunks, name, write_name, load, exe, length)
    
    Export the list of chunks given as a file on the path specified with
    the name given as write_name. The original name is also supplied for
    use in .inf files.
    """
    ...
    if inf_file != None:
        ...
        while position < end_pos:
            ...
            if position == None:
                ...
                name, load, exec_addr, data, block_number, last = \
                    read_block(chunks[position])
...
def decode_chunk(out_path, chunk_info, position):
    ...
    """decode_chunk(out_path, chunk_info, position)
    
    Write a file depending on the chunk number using a convention similar to
    that used for encoding chunks.
    """
    
    decode_as = \
    {
        0x0: 'creator', 0x1: 'manual', 0x2: 'credits', 0x3: 'inlay',
    ...
    # If the chunk number is not in the dictionary then write a file with
    # name constructed from the hexadecimal form of the chunk number
    # beginning with 0x.
    ...
    if decode_as.has_key(chunk_info[0]):
        ...
        # Attempt to save the chunk in a file with a name made up from the
        # position of the chunk in the list with a hexadecimal suffix.
    ...
    try:
        ...
        print "Couldn't write file %s for chunk number %s." % (
            name, hex(chunk_info[0])
            )
...
def printable(s):
    ...
    Return a printable form of the string supplied which replaces ASCII
    characters with codes less than 32 with question mark symbols.
    """
...
def browsable(s):
    ...
    Return a form of the string supplied which replaces ASCII characters
    with codes less than 32 or greater than 126 with question mark symbols
    and encodes any special characters for use in an HTML page.
    """
...
(    
  <UEF file>
   ...
   |
     ( "wwwinfo" Directory )
   |
     ( "new" <Machine type> <Keyboard type> [<UEF version>] )
   |
       "cat"
   |
     ( "append" Files )
   |
     ( "insert" Position <Files or chunks> )
   |
     ( "remove" Positions )
   |
     ( "extract" Positions <Output path> )
   |
       "chunks"
)
  | ( "help" Command )
...
"""
    ...
    if len(matches) != 1 and cmdsyntax.use_GUI() is not None:
    ...
    if match.has_key('help'):
        ...
        print_help(match['Command'])
        
        # Exit
        sys.exit()
    
    # All other operation involved a UEF file.
    ...
    originator = target_machine = keyboard_layout = 'Unknown'
    
    # New command (create the file with the name given)
    
    if match.has_key('new'):
    
        # If there is an argument specifying the machine type then add
        # a chunk for this
        
        target_machine = match['Machine type']
        keyboard_layout = match['Keyboard type']
        
        if match.has_key('UEF version'):
            write_version = match['UEF version']
        else:
            write_version = '0.9'
    ...
    if match.has_key('extract'):
    
        output_path = match['Output path']
        
        # Check whether the output path is a directory or a UEF file
        if string.lower(output_path[-4:]) == suffix+'uef':
        
            # Check whether the file already exists.
            try:
                open(output_path).close()
            
            except IOError:
            
                try:
                    dest_uef = gzip.open(output_path, 'wb')
                except IOError:
                    print "Couldn't open file %s for writing." % output_path
                    sys.exit()
    ...
    #            uef.close()
            ...
            leafname = get_leafname(output_path)
    ...
    if match.has_key('wwwinfo'):
    
        output_path = match['Directory']
...
#                    current_file['last position'] = find_file_end(chunks, position)
                    ...
                    current_file = \
                    {
                        'name': name, 'load': load, 'exec': exec_addr,
                        'blocks': block_number, 'data': data
                    }
                    ...
                    # Update the last position information to mark the end of
                    # the file.
                    current_file['last position'] = position
    ...
    if match.has_key('wwwinfo'):
        ...
        index_file = output_path+os.sep+'index'+suffix+'html'
        try:
        ...
        if instructions != None:
            ...
            # Split text at paragraph breaks.
        ...
        if inlay != None:
            ...
            inlay_path = output_path + os.sep + 'inlay'
            index.write(
    ...
    if match.has_key('insert'):
        
        # There are two versions of this command: one inserts files, the
        # other inserts chunks.
        #
        # The first argument determines which version is used:
        #
        #     1) If the position has a "c" at the beginning then we interpret
        #        the second argument as a list of files to be treated as
        #        chunks.
        #
        #     2) If the position doesn't have a "c" at the beginning then we
        #        interpret the second argument as a file position and have to
        #        determine the chunk position before we can insert the files.
        
        if match['Position'][0] == 'c':
        
            # We are inserting chunks.
            try:
                position = int(match['Position'][1:])
            except ValueError:
                print insert_syntax
                sys.exit()
            ...
            file_names = string.split(match['Files or chunks'], ',')
            
            # Insert the chunks in the list at the specified position
            chunks = chunks[:position] + encode_chunks(file_names) + \
                chunks[position:]
            ...
            # File position
            try:
                file_position = int(match['Position'])
            except ValueError:
                print insert_syntax
                sys.exit()
    ...
    if match.has_key('append'):
    
        # Names of files to insert (comma-separated list)
        file_names = string.split(match['Files'], ',')
    ...
    if match.has_key('extract'):
        ...
        positions = string.split(match['Positions'], ',')
        
        # Destination path in which to put the files (any directories
        # should have already been created).
        out_path = match['Output path']
        
        # This command will extract chunks from the list of chunks using a
        # chunk position given and extract files using the file position
        # which is converted to a chunk position first.
...
#                        write_chunks(gzip.open(out_path, 'ab'), chunks[position:position+1])
                        ...
                        print "Couldn't write to file %s for chunk %i." % (
                            out_path, position
                            )
                else:
                ...
                # Find the chunk position which corresponds to the file
                # position.
                if file_position < 0 or file_position >= len(contents):
                
                    print 'File position %i does not ' % file_position + \
...
#                            write_chunks(gzip.open(out_path, 'ab'), chunks[start_pos:end_pos+1])
                            ...
                            print "Couldn't write file %i to archive %s" % (
                                file_position, out_path
                                )
                    else:
                    
                        # Writing a file in the archive to a file in a
                        # directory.
                        ...
                        export_file(
                            out_path, chunks[start_pos:end_pos+1], name,
    ...
    if match.has_key('remove'):
    
        # As with the insert command, there are two versions of this command.
        # The first will remove chunks from the list using a chunk position
        # given, the second will remove files using the file position which
        # is converted to a chunk position first.
        
        # File positions of files to extract.
        file_positions = string.split(match['Positions'], ',')
...
#            sys.exit()
        ...
        for file_position in file_positions:
            ...
            if file_position[0][0] == 'c':
                ...
                try:
                    # Append the chunk number to the list of chunks to leave
                    # out.
                ...
                if file_position < 0 or file_position >= len(contents):
                    ...
                    print 'File position %i ' % file_position + \
                        'does not correspond to an actual file.'
